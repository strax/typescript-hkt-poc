...
interface Identity$kind extends Kind1<Identity$witness> {
  [Kind1.refine]: this extends Type1<Identity$kind, infer A>
    ? Identity<A>
    : never;
}
...
interface Const$kind extends Kind2<Const$witness> {
  [Kind2.refine]: this extends Type2<Const$kind, infer T1, infer T2>
    ? Const<T1, T2>
    : never;
}
...
interface Forget$kind extends Kind3<Forget$witness> {
  [Kind3.refine]: this extends Type3<Forget$kind, infer T1, infer T2, infer T3>
    ? Forget<T1, T2, T3>
    : never;
}
...
declare function inj2<F extends Kind2, A, B>(
  fa: Type2<F, A, B>
): Type2<F, A, B>;
declare function prj2<F extends Kind2, A, B>(fa: Type2<F, A, B>): Fix2<F, A, B>;
declare function fix2<F extends Kind2, A, B>(
  fa: Fix2<F, A, B>
): Fix2<F, ["A", A], ["B", B]>;

declare function inj3<F extends Kind3, A, B, C>(
  fa: Type3<F, A, B, C>
): Type3<F, A, B, C>;
declare function prj3<F extends Kind3, A, B, C>(
  fa: Type3<F, A, B, C>
): Fix3<F, A, B, C>;
declare function fix3<F extends Kind3, A, B, C>(
  fa: Fix3<F, A, B, C>
): Fix3<F, ["A", A], ["B", B], ["C", C]>;
